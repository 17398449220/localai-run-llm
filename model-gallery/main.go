...
func getModel(modelID string) (HFModel, error) {
	...
	resp, err := http.Get(fmt.Sprintf("https://hf-mirror.com/api/models/%s", modelID))
	if err != nil {
		return files, err
	}
...
func getModelFiles(repository string, modelFiles HFModel) (HFModel, error) {
	...
	for _, sibling := range modelFiles.Siblings {
		...
		fileURL := fmt.Sprintf("https://hf-mirror.com/%s/resolve/main/%s", repository, sibling.RFileName)
		shaURL := fmt.Sprintf("https://hf-mirror.com/%s/blob/main/%s", repository, sibling.RFileName)
...
func scraperWorker(wg *sync.WaitGroup, c chan string, g chan GalleryModel) {
	...
	for model := range c {
		...
		for _, m := range mm.Files {
			...
			g <- GalleryModel{
				Name:    modelName,
				URLs:    []string{fmt.Sprintf("https://hf-mirror.com/%s", model)},
...
func scrapeHuggingFace(term string, concurrency int, indexFile string) {
	...
	resp, err := http.Get(fmt.Sprintf("https://hf-mirror.com/api/models?search=%s", term))
	if err != nil {
		log.Fatal(err)
	}
	defer resp.Body.Close()
